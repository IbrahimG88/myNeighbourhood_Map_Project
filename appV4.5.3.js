    ...
    self.filteredItems = ko.computed(function () {
        ...
        if (!filter) {
            ...
            return ko.utils.arrayFilter(Model, function (item) {
                var visibleplacesTitle = item.title.toLowerCase().indexOf(filter) !== -1;
                item.marker.setVisible(visibleplacesTitle);
                var visibleplacesType = item.type.toLowerCase().indexOf(filter) !== -1;
                item.marker.setVisible(visibleplacesType);
                return  visibleplacesTitle && visibleplacesType;
            });
        }
    }, self);
